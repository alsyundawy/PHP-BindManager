...
final class RecordValidator
{
    ...
    public function validate(array $data): array
    {
        ...
        if ($name === '' || strlen($name) > 253) {
            $errors['name'][] = 'Record owner name is invalid.';
        }

        if (! in_array($type, $allowed, true)) {
            $errors['record_type'][] = 'Unsupported record type.';
        }

        if ($content === '' || strlen($content) > 65535) {
            $errors['content'][] = 'Record content is required and must be <= 65535 bytes.';
        }

        if ($ttl < 1 || $ttl > 2147483647) {
            $errors['ttl'][] = 'TTL must be between 1 and 2147483647.';
        }
